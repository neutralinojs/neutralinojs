...
#define ID_MENU_FIRST 20000
    ...
    LRESULT CALLBACK NeutralinoWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        if (msg == WM_DROPFILES)
        {
    ...
    void setBorderless(bool borderless)
    {
        ...
        unsigned long windowStyleMask = ((unsigned long (*)(id, SEL))objc_msgSend)(
            (id)windowHandle, "styleMask"_sel);
        windowStyleMask = borderless ? (windowStyleMask & ~NSWindowStyleMaskTitled) : (windowStyleMask | NSWindowStyleMaskTitled);
        ((void (*)(id, SEL, int))objc_msgSend)((id)windowHandle,
                                               "setStyleMask:"_sel, windowStyleMask);
#elif defined(_WIN32)
        DWORD currentStyle = GetWindowLong(windowHandle, GWL_STYLE);
        currentStyle = borderless ? (currentStyle & ~(WS_CAPTION | WS_THICKFRAME)) : (currentStyle | (WS_CAPTION | WS_THICKFRAME));
        SetWindowLong(windowHandle, GWL_STYLE, currentStyle);
        SetWindowPos(windowHandle, NULL, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
#endif
    }
    ...
    bool snapshot(const string &filename)
    {
        ...
        bool status = ((bool (*)(id, SEL, id, bool))objc_msgSend)(screenshotData, "writeToFile:atomically:"_sel,
                                                                  ((id (*)(id, SEL, const char *))objc_msgSend)("NSString"_cls, "stringWithUTF8String:"_sel, filename.c_str()), true);
