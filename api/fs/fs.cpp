...
namespace fs
{
    ...
    bool updateOpenedFile(const OpenedFileEvent &evt)
    {
        ...
        else if (evt.type == "read" || evt.type == "readAll" || evt.type == "readBinary" || evt.type == "readAllBinary")
        {
    ...
    namespace controllers
    {
        ...
        json updateOpenedFile(const json &input)
        {
            ...
            {
                if (fileEvt.type == "read" || fileEvt.type == "readAll" || fileEvt.type == "readBinary" || fileEvt.type == "readAllBinary")
                {
        ...
        json chmod(const json &input)
        {
            ...
            {
                output["error"] = errors::makeMissingArgErrorPayload("path or mode");
                return output;
            }
            ...
            error_code ec;
            // Converts numeric 755/644 to filesystem permissions
            filesystem::perms p = static_cast<filesystem::perms>(
                (mode / 100) << 6 | ((mode / 10) % 10) << 3 | (mode % 10));

            filesystem::permissions(CONVSTR(path), p, filesystem::perm_options::replace, ec);
        ...
        json getWatchers(const json &input)
        {
            ...
            {
                output["returnValue"].push_back({{"id", watcherId},
                                                 {"path", info.second}});
            }
            output["success"] = true;
            return output;
        ...
        json setPermissions(const json &input)
        {
            ...
            if (helpers::hasField(input, "mode"))
            {
                ...
                if (mode == "ADD")
                    permMode = filesystem::perm_options::add;
                if (mode == "REPLACE")
                    permMode = filesystem::perm_options::replace;
                if (mode == "REMOVE")
                    permMode = filesystem::perm_options::remove;
            }
