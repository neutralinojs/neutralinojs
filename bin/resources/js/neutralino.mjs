...
function screenshot(filename) {
    return sendMessage('window.captureScreen', { outputFile: filename });
}
...
function readImage(format = '') {
    return new Promise((resolve, reject) => {
        sendMessage('clipboard.readImage')
            .then((image) => {
            if (image) {
                const binaryData = window.atob(image.data);
                let numOfBytes = image.bpp == 32 ? 4 : 3;
                let len, pattern, bytes;
                ...
                bytes = new Uint8Array(len);
                let isLittleEndian = new Uint8Array(new Uint32Array([0x000000ff]).buffer)[0] == 0xff;
                let byteA, byteB, byteC, byteD, uint32;
                let rgba = [];
                let outIndex = 0;
